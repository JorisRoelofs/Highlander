/// @description: 
scale = 1;// + (0.3*charge);
deathTime = ((0.8*charge) + 0.1) * deathTimeMax;
if(instance_exists(owner) && instance_exists(objPlayer) && owner.id = objPlayer.id) scr_shake(max(1, 5 * charge), (deathTime + 0.1) * room_speed);
else
{
